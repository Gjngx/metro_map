...
function TrainDetail() {
  ...
  const deleteTrain = () => {
    ...
    if (confirmDelete) {
      TrainService.deleteTrain(id)
        .then((response) => {
          navigate('/admin/Tàu/'+train.idTuyenTau);
        })
        .catch((error) => {
          console.log(error);
        });
  ...
  <div className="train-line-detail">
    {train && Object.keys(train).length > 0 ? (
      <div>
        <h2>Chi tiết ga</h2>
        <p className="label">Số tuyến tàu: {train.soTuyenTau}</p>
    ...
    <button className="btn-addtrainline btn-success float-right margin-2" onClick={deleteTrain}>
      Xóa ga
    </button>
    <Link style={{ textDecoration: 'none' }} to= {"/admin/Tàu/"+ train.idTuyenTau} className="btn-addtrainline btn-danger float-right margin-2">
      Hủy
    </Link>
  </div>
  );
